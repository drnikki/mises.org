 ...
 */
function giving_preprocess_region(&$variables) {
  global $theme;
  static $wells;
  if (!isset($wells)) {
    foreach (system_region_list($theme) as $name => $title) {
      $wells[$name] = theme_get_setting('giving_region_well-' . $name);
    }
  }
  ...
  switch ($variables['region']) {
    ...
    case 'help':
      $variables['content'] = _giving_icon('question-sign') . $variables['content'];
